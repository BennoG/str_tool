...
/*
static void MD5_print(MD5_CTX *context)
{
	printf("count: %d %d\n",context->count[0],context->count[1]);
	printf("state: %d %d %d %d\n",context->state[0],context->state[1],context->state[2],context->state[3]);
	printf("data: %02X %02X %02X %02X %02X %02X %02X %02X\n",context->buffer[0],context->buffer[1],context->buffer[2],context->buffer[3],context->buffer[4],context->buffer[5],context->buffer[6],context->buffer[7]);
}
*/
...
static void MD5FinalSt(char st[33], MD5_CTX *context)
{
	char *p=st;
	int i;
	uint8_t digest[16];
	MD5Final (digest,context);
	for (i=0;i<16;i++){sprintf(p,"%02x",digest[i]);p+=2;}
}
...
STP stlMd5Mem(const char *sBuf,int iBufLen)
{
    ...
    MD5_CTX md5;
	if (sBuf==NULL) sBuf="";
	if (iBufLen<0) iBufLen=strlen(sBuf);
    MD5Init(&md5);
//MD5_print(&md5);
	MD5Update(&md5,(const uint8_t*)sBuf,(unsigned int)(iBufLen));
//MD5_print(&md5);
	sMd5=stlInitLen(32,0);
    MD5FinalSt(sMd5->sBuf,&md5);
	return sMd5;
}
STP stlMd5Stp(STP sData)
{
	if (sData)return stlMd5Mem(sData->sBuf,sData->iLen);
	return stlMd5Mem("",0);
}
...
static void Encode (uint8_t *output, uint32_t *input, unsigned int len)
{
	...
	for (i = 0, j = 0; j < len; i++, j += 4) {
		output[j+0] = (uint8_t)(input[i] & 0xff);
