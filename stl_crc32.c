...
/************************************************************************
There are two differences, the main difference is that CRC-32 is a reflected algorithm and CRC-32/MPEG is unreflected. In MPEG, bits are sampled at the top of the register instead of the bottom. E.g. where a bitwise algorithm has
Code:
   if(crc & 1) 
...
The other difference is that the register is not XORed with 0xffffffff before it is returned. 
***********************************************************************/
...
static unsigned long crc_tableMPG[256] = {
...
unsigned long stlCrc32MemMPG (const void *buf, size_t len)
{
	...
	for (i=0; i<len; i++)
		crc = (crc << 8) ^ crc_tableMPG[((crc >> 24) ^ *data++) & 0xff];
	return crc;
}
 ...
 *	name    name fo file to calulate crc from
...
int stlCrc32File(const char *name, unsigned long *crc, long *charcnt)
{
   ...
   if (ferror(fin))
   {
      fprintf(stderr,"Bestand fout\n");
      cnt=-1;
      ret=3;
   }
   fclose(fin);

   if (crc)  *crc = oldcrc32 = ~oldcrc32;
   if (charcnt) *charcnt=cnt;
 ...
 * Return crc of buffer
 */
unsigned long stlCrc32Mem(const void *data, size_t len)
{
   register unsigned long oldcrc32;
...
{
	if (sData) return stlCrc32Mem(sData->sBuf,sData->iLen);
	return stlCrc32Mem("",0);
}
